...
const EventDescription = () => {
  ...
  const handleClick = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    EventService.subscribe(eventId || '')
      .then(() => {
        setIsLoading(false);
        showToast(true, 'success', 'Event subscribed successfully!');
  ...
  const isSubscribeDisabled = (event?: IEvent): { disabled: boolean; message: string } => {
    const userId = currentUserId();
    if (!event) return { disabled: true, message: 'No data' };

    if (event.subscribers.includes(userId))
      return { disabled: true, message: 'Already Subscribed to this event' };

    if (userId === event.creatorId)
      return { disabled: true, message: '*Cannot subscibe your own events' };
                  ...
                  {eventStatus == 'upcoming' && (
                    <Stack
                      ...
                      <Box display={'flex'} flexDirection={'column'}>
                        <LoadingButton
                          sx={{ padding: '12px', color: '#fff', borderRadius: '0' }}
                          loading={isLoading}
                          onClick={handleClick}
                          disabled={isSubscribeDisabled(eventData).disabled}
                          variant="contained">
                          <Fragment>
                        ...
                        {isSubscribeDisabled(eventData).disabled && (
                          <Typography
                            fontWeight={'400'}
                            ...
                            variant="caption"
                            align="left">
                            {isSubscribeDisabled(eventData).message}
                          </Typography>
                        )}
                      </Box>
                    </Stack>
